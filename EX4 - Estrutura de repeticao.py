"""Supondo que a população de um país A seja da ordem de 80000 habitantes
com uma taxa anual de crescimento de 3% e que a população de B seja 
200000 habitantes com uma taxa de crescimento de 1.5%. 
Faça um programa que calcule e escreva o número de anos necessários 
para que a população do país A ultrapasse ou iguale a população do 
país B, mantidas as taxas de crescimento."""

pop_A= 80000
pop_B= 200000
anos = 0

# print('A população A possui 80 mil Habitantes, enquanto a população B 200 mil habitantes!\n')

while pop_A <= pop_B:
     pop_A= float(round(pop_A + (pop_A* 0.03)))
     pop_B= float(round(pop_B + (pop_B* 0.015)))
     anos= anos + 1
else:
    print(f'Será necessário {anos} anos para a população A passar a população B.')
    